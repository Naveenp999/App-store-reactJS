...
class AppStore extends Component {
  state = {Id: 'SOCIAL', text: ''}

  getAppList = () => {
    const {Id} = this.state
    return appsList.filter(element => element.category === Id)
  }

  makechanges = name => this.setState({Id: name})

  inputchange = event => this.setState({text: event.target.value})
        ...
        <div className="sub-container">
          ...
          <ul className="tab-container">
            {tabsList.map(element => (
              <TabItem
                ...
                isActive={element.tabId === Id ? true : false}
                key={element.tabId}
              />
            ))}
          </ul>
          ...
          <ul className="app-container">
            {applist.map(
              element =>
                check(element) && (
                  <AppItem content={element} key={element.appId} />
                ),
            )}
          </ul>
        </div>
